...
contract MinterBreeding is MinterCore, OwnableUpgradeable, UUPSUpgradeable {
     ...
     */
    function breed(uint256[] calldata parents)
        public
     ...
     * @dev Create a new type of species and define attributes.
     */
    function safeBreed(uint256[] calldata parents)
        public
        returns (uint256 tokenId)
    {
        // Check if user enabled generational species
    ...
    function setBreedingRules(
        ...
        for (uint256 i = 0; i < mutationRates.length; i++)
     ...
     */
    function _breedSpecies(uint256[] calldata parents, address caller)
        internal
        returns (uint256 tokenId)
    {
        ...
        uint256[] memory mutationRates;
        uint256 breedCooldownSeconds;
        uint16 generationCooldownMultiplier = _breedingRules
        ...
        require(
            parents.length == requiredParents,
        ...
        if (mutationRates.length == 0)
            ...
            tokenId = RosalindDNA.breedDNAWithMutations(
                parents,
                genes,
                randomSeed,
                mutationRates
        ...
        )
    {
        ...
        if (breedCooldownSeconds == 0)
            breedCooldownSeconds = defaultBreedingCooldownSeconds;
        ...
        if (rules.genes.length == 0)
            ...
            for (uint256 i = 0; i < defaultGenesNum; i++)
        ...
        if (mutationRates.length != 0) {
            require(
                mutationRates.length == genes.length,
                "mutation rates length and genes length must match."
            );
