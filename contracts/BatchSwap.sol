...
abstract contract CPInterface {
    function transferPunk(address to, uint256 index) public virtual;
...
contract PunkProxy {
    ...
    function proxyTransferPunk(
        address _punkContract,
        address _to,
        uint256 _punkIndex
    ) public {
        require(owner == msg.sender, "You're not the contract owner");
        require(
            CPInterface(_punkContract).punkIndexToAddress(_punkIndex) ==
                address(this),
            "Punk is missing from this Proxy"
        );
...
contract BatchSwap is Ownable, Pausable, IERC721Receiver, IERC1155Receiver {
    ...
    function createSwapIntent(
        ...
        for (i = 0; i < nftsOne[_swapIntent.id].length; i++) {
            ...
            } else if (nftsOne[_swapIntent.id][i].typeStd == CRYPTOPUNK) {
                // Controllo che il CP sia presente sul proxy e che non sia in uso in un altro trade
                require(
                    punkInUse[nftsOne[_swapIntent.id][i].tokenId[0]] == false,
                    "Punk in use on another trade"
                );
                require(
                    CPInterface(CRYPTOPUNK).punkIndexToAddress(
                        nftsOne[_swapIntent.id][i].tokenId[0]
                    ) == address(punkProxies[msg.sender]),
                    "CryptoPunk missing"
                );
        ...
        whenNotPaused
    {
        ...
        for (i = 0; i < nftsOne[_swapId].length; i++) {
            ...
            } else if (nftsOne[_swapId][i].typeStd == CRYPTOPUNK) {
                // Controllo che il CP sia su questo smart contract
                require(
                    CPInterface(CRYPTOPUNK).punkIndexToAddress(
                        nftsOne[_swapId][i].tokenId[0]
                    ) ==
                        address(
                            punkProxies[
                                swapList[_swapCreator][swapMatch[_swapId]]
                                    .addressOne
                            ]
                        ),
                    "CryptoPunk missing"
    ...
    function cancelSwapIntent(uint256 _swapId) public {
        ...
        for (i = 0; i < nftsOne[_swapId].length; i++) {
            if (nftsOne[_swapId][i].typeStd == ERC20) {
                ...
                // Controllo che il CP sia presente sul proxy
                require(
                    CPInterface(CRYPTOPUNK).punkIndexToAddress(
                        nftsOne[_swapId][i].tokenId[0]
                    ) == address(punkProxies[msg.sender]),
                ...
                punkProxies[msg.sender].proxyTransferPunk(
                    CRYPTOPUNK,
                    msg.sender,
                    nftsOne[_swapId][i].tokenId[0]
                );
    ...
    function registerPunkProxy() public {
        require(
            address(punkProxies[msg.sender]) == address(0),
    ...
    function claimPunkOnProxy(uint256 _punkId) public {
