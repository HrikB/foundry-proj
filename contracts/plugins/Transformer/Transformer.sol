    ...
    ERC1155HolderUpgradeable
{
    ...
    bytes4 private constant ERC165TAG =
        bytes4(
            keccak256(abi.encodePacked("OWLProtocol://Transformer/", version))
        );
    ...
    **********************/
    ...
    function __Transformer_init(
        ...
        require(
            ...
            "Transformer: burn address must not be 0"
        );
        require(
            _inputs.length > 0,
            "Transformer: A crafting input must be given!"
        );
    ...
    function transform(uint256 tokenId, uint256[][] calldata _inputERC721Ids)
        external
    {
        require(
        ...
        for (uint256 i = 0; i < inputs.length; i++) {
            ...
            if (ingredient.token == PluginsLib.TokenType.erc20) {
                //ERC20
                if (
                    ingredient.consumableType ==
                    PluginsLib.ConsumableType.burned
                ) {
                    //Transfer ERC20
                ...
                if (
                    ...
                    for (uint256 j = 0; j < currInputArr.length; j++) {
                        IERC721Upgradeable(ingredient.contractAddr)
                            .safeTransferFrom(
                                _msgSender(),
                                burnAddress,
                    ...
                    for (uint256 j = 0; j < currInputArr.length; j++) {
                        require(
                            IERC721Upgradeable(ingredient.contractAddr).ownerOf(
                                currInputArr[j]
                            ) == _msgSender(),
                        ...
                        require(
                            (usedERC721Inputs[ingredient.contractAddr])[
                                currTokenID
                            ] < nUse[i],
