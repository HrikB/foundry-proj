...
import "../../contracts/OwlBase.sol";
import "../../contracts/random/VRFBeacon.sol";
import "../../contracts/plugins/PluginsLib.sol";
import "../../contracts/utils/SourceRandom.sol";
import "../../contracts/utils/Probability.sol";

contract RouteRandomizer is
    OwlBase,
    ...
    ERC1155HolderUpgradeable
{
    ...
    bytes4 private constant ERC165TAG =
        bytes4(
            keccak256(
                abi.encodePacked("OWLProtocol://RouteRandomizer/", version)
            )
        );
    ...
    **********************/
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        uint256 randomness = VRFBeacon(vrfBeacon).getRandomness(
            elements[queueIndex].epochBlock
        );
    ...
    function performUpkeep(bytes memory performData) external override {
        (uint256 randomness, uint256 queueIndexRequest) = abi.decode(
            performData,
            (uint256, uint256)
        );
        ...
        require(
            queueIndexRequest == queueIndex,
            "Lootbox: queueIndex already processed"
        );
    ...
    function _routeRandomize(bytes[] memory argsArr, uint256 randomSeed)
        internal
    {
        //randomly choose the crafter transfer contract to call
        uint256 randomNumber = SourceRandom.getSeededRandom(
            randomSeed,
            queueIndex
        );
        uint256 selectedContract = Probability.probabilityDistribution(
            randomNumber,
            probabilities
        );
