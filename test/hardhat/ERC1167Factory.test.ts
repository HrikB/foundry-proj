...
describe("ERC1167Factory", () => {
  ...
  beforeEach(async () => {
    ERC1167FactoryFactory = (await ethers.getContractFactory(
      "ERC1167Factory"
  ...
  it("clone()", async () => {
    ...
    const { instance: instance1 } = pick(events1[0].args, [
      "instance",
      "implementation",
      "salt",
    ]);

    const contrInst1 = (await ethers.getContractAt(
      "ERC721Owl",
      instance1
    )) as ERC721Owl;
    ...
    const tx2 = await ERC1167Factory.clone(
      ERC721Impl.address,
      ERC721Impl.interface.encodeFunctionData("initialize", [
    ...
    const {
      ...
      implementation,
      salt,
    } = pick(events2[5].args, ["instance", "implementation", "salt"]);

    expect(implementation).to.equal(ERC721Impl.address);
    expect(salt).to.equal(hexZeroPad("0x00", 32));

    const contrInst2 = (await ethers.getContractAt(
      "ERC721Owl",
      instance2
    )) as ERC721Owl;
  ...
  it("cloneDeterministic", async () => {
    ...
    const tx1 = await ERC1167Factory.cloneDeterministic(
      ERC721Impl.address,
      saltBytes,
      "0x"
    );
    ...
    const tx2 = await ERC1167Factory.cloneDeterministic(
      ERC721Impl.address,
      saltBytes,
      data
    );
    ...
    const { instance: instance2, implementation } = pick(events2[5].args, [
      "instance",
      "implementation",
    ]);
